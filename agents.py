...
from utils import DataUtils, SetParamsAsAttributes, as_tensor


class MPCAgent(SetParamsAsAttributes):
    def __init__(self, params):
        ...
        self.save_paths = [os.path.join(self.state_dict_save_dir, f"robot_{'_'.join([str(id) for id in self.robot_ids])}_object_{self.use_object}_state_dict_{i}.pt") for i in range(self.ensemble_size)]
        self.scale_idx_path = os.path.join(self.state_dict_save_dir, f"robot_{'_'.join([str(id) for id in self.robot_ids])}_object_{self.use_object}_scale_idx.npy")
        ...
        for t in range(mpc_horizon):
            ...
            with torch.no_grad():
                for i, model in enumerate(self.models):
                    ...
                    sample = self.ensemble_size > 1 and self.n_particles > 1
                    pred_next_states[:, start:end] = model(cur_state, action, sample=sample, delta=False).reshape(mpc_samples, self.n_particles // self.ensemble_size, self.state_dim)
    ...
    def compute_trajectory_costs(self, predicted_state_sequence, sampled_actions, goals, initial_state):
        ...
        for cost_type, cost_weight in self.cost_weights_dict.items():
            # if cost_weight != 0. and cost_type != 'distance':
            if cost_weight != 0.:
                ensemble_costs_per_step += cost_dict[cost_type] * cost_weight

        # ensemble_costs_per_step = (ensemble_costs_per_step + self.cost_weights_dict['distance']) * cost_dict['distance']
        ...
        ensemble_costs_per_sample = ensemble_costs_per_sample.mean(dim=2)
        costs_per_sample = ensemble_costs_per_sample.mean(dim=0)
        # costs_per_sample = ensemble_costs_per_sample.mean(dim=(0, 2))

        if self.use_object:
            # ensemble_costs_std = ensemble_costs_per_sample.std(dim=(0, 2))
            ensemble_costs_std = ensemble_costs_per_sample.std(dim=0)
...
class RandomShootingAgent(MPCAgent):
    ...
    def get_action(self, initial_state, goals):
        ...
        predicted_next_state = predicted_state_sequence[:, best_idx, :, 0].mean(dim=(0, 2)).squeeze().detach().cpu().numpy()

        self.best_cost = total_costs[best_idx]
...
class CEMAgent(MPCAgent):
    ...
    def get_action(self, initial_state, goals):
        ...
        for i in range(self.cem_refine_iters):
            if i > 0:
            ...
            best_costs_idx = np.argsort(total_costs)[:self.cem_n_best]
            ...
            trajectory_mean = self.cem_alpha * best_trajectories_mean + (1 - self.cem_alpha) * trajectory_mean
            trajectory_std = self.cem_alpha * best_trajectories_std + (1 - self.cem_alpha) * trajectory_std
        ...
        # best_action = np.random.normal(loc=trajectory_mean[0], scale=trajectory_std[0], size=self.action_dim)
        best_action = np.clip(best_action, -1., 1.)
        predicted_next_state = self.simulate(initial_state, best_action[None, None, :]).mean(dim=(0, 2)).squeeze().detach().cpu().numpy()
...
class MPPIAgent(MPCAgent):
    ...
    def get_action(self, initial_state, goals):
        ...
        noise = np.random.normal(loc=0, scale=self.mppi_noise_std, size=(self.mpc_samples, self.mpc_horizon, self.action_dim))

        for t in range(self.mpc_horizon):
            previous_action = just_executed_action if t == 0 else sampled_actions[:, t-1]
            sampled_actions_t = self.mppi_beta * (self.trajectory_mean[t] + noise[:, t]) + (1 - self.mppi_beta) * previous_action
        ...
        weights = np.exp(self.mppi_gamma * (total_rewards - total_rewards.max()))
