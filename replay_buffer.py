...
from utils import SetParamsAsAttributes, as_tensor
...
class ReplayBuffer(SetParamsAsAttributes):
    def __init__(self, params, precollecting=False):
        ...
        self.goals = np.empty((self.buffer_capacity, STATE_DIM))
        self.next_goals = np.empty((self.buffer_capacity, STATE_DIM))
        self.rewards = np.empty((self.buffer_capacity, 1))
        ...
        self.save_path = os.path.join(self.save_dir, f"{self.exp_name}.npz")

    @property
    def size(self):
        return self.buffer_capacity if self.full else self.idx

    def add(self, state, action, next_state, goal=None, next_goal=None, reward=None):
        ...
        if next_goal is not None:
            np.copyto(self.next_goals[self.idx], goal)
    ...
    def sample_rl(self, sample_size):
        sample_size = min(self.size, sample_size)
        stored_idxs = np.arange(self.size)
        sample_idx = np.random.choice(stored_idxs, sample_size, replace=False)

        states = self.states[sample_idx]
        actions = self.actions[sample_idx]
        next_states = self.next_states[sample_idx]
        goals = self.goals[sample_idx]
        next_goals = self.next_goals[sample_idx]
        rewards = self.rewards[sample_idx]

        return as_tensor(states, goals, next_states, next_goals, actions, rewards)
